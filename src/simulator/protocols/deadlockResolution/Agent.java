...
public class Agent {
    ...
    public void resolve() {
        if(Log.isLoggingEnabled()) log.log(agentID,deadlockID+": Resolving deadlock for trans " + agentID + " for deadlock:" + transactionsInDeadlock);
...
//        if(Log.isLoggingEnabled()) log.log("; workload = " + workload);
        ...
        transactionsInDeadlock.forEach(ti -> {
            ...
            if( ti.transID != agentID ){
                if(Log.isLoggingEnabled()) log.log(agentID, deadlockID+": Sending dropability ("+dropability+") to other agent " + ti.transID);
    ...
    public void receiveDropability(int dropability, int transID){
        if(Log.isLoggingEnabled()) log.log(this.agentID, deadlockID+":Agent: receive Dropability (dropability = [" + dropability + "], transID = [" + transID + "])");
        ...
        if( agentsToDropabilities.values().size() == transactionsInDeadlock.size()-1 ){
            ...
            if( maxDropability < this.dropability){
                ...
                if( simParams.timeProvider.get() + myTrans.executionTime > myTrans.deadline ){
...
//                        if(Log.isLoggingEnabled()) log.log(agentID, "Agents determined that " + agentID + ",  should be dropped.");
                ...
                if(Log.isLoggingEnabled()) log.log(agentID, deadlockID+": Agents determined that " + agentID + "  should be dropped.");
                ...
                if( count(agentsToDropabilities.values(), this.dropability) > 1){
                    ...
                    if( count(priorities, lowestPriority ) > 1){
                        ...
                        dropables.forEach(d -> {if( d.getPriority() == lowestPriority) lowestPriorities.add(d);});
                        ...
                        if( count(workloads, lowestWorkload ) > 1){
                            ...
                            lowestPriorities.forEach(d -> {if( d.workload == lowestWorkload) lowestWorkloads.add(d);});
                            ...
                            if( youngest.serverID == server.getID() ){
                                if(Log.isLoggingEnabled()) log.log(agentID, deadlockID+": Agents determined that " + youngest.transID + " should be dropped.");
                                simParams.getDeadlockResolutionListener().accept(deadlock, agentID);
                                server.getTM().abort(youngest.transID );
                            }
                            else{
                                if(Log.isLoggingEnabled()) log.log(agentID, deadlockID+": Agents determined that " + youngest.transID + " should be dropped, but it not on this server!");
                            }
                            return;
                        }
                            ...
                            dropables.forEach(dropable -> {
                                if( dropable.workload == lowestWorkload ){
                                    if( dropable.serverID == server.getID() ) {
                                        if(Log.isLoggingEnabled()) log.log(agentID, deadlockID+": Agents determined that " + dropable.transID + "Should be dropped.");
                                        simParams.getDeadlockResolutionListener().accept(deadlock, agentID);
                                        server.getTM().abort(dropable.transID);
                                    }
                                    else{
                                        if(Log.isLoggingEnabled()) log.log(agentID, deadlockID+": Agents determined that " + dropable.transID + "Should be dropped, but its not on this server!");
                                    }
                                }
                            });
                            return;
