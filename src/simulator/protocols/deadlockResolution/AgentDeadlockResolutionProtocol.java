...
public class AgentDeadlockResolutionProtocol implements DeadlockResolutionProtocol{
    ...
    public void receiveMessage(Message message) {
        ...
        switch (msg[0]){
            case "RD": {
                if(Log.isLoggingEnabled()) log.log(Integer.parseInt(msg[1]), "Receive message: " + message);
    ...
    public void receiveDropability(int transID, int agentID, int deadlockID, int dropability) {
        if(Log.isLoggingEnabled()) log.log(transID,"Received dropability:" + dropability + " from agent: " + agentID + " for deadlock: " + deadlockID);
